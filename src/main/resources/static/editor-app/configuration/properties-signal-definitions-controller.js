    ...
    ['$scope', '$q', '$translate', '$timeout', function ($scope, $q, $translate, $timeout) {
        ...
        if ($scope.property.value !== undefined && $scope.property.value !== null && $scope.property.value.length > 0) {
            ...
            if ($scope.property.value.constructor == String) {
                $scope.fieldAttributes = JSON.parse($scope.property.value);
            }
            else {
                // Note that we clone the json object rather then setting it directly,
                // this to cope with the fact that the user can click the cancel button and no changes should have happened
                $scope.fieldAttributes = angular.copy($scope.property.value);
            }
        ...
        //     $scope.fieldAttributes.push(newSignalDefinition);
        //     $timeout(function () {
        //         $scope.gridOptions.selectItem($scope.fieldAttributes.length - 1, true);
        //     });
        // };
        ...
        //         var index = $scope.fieldAttributes.indexOf($scope.selectedSignals[0]);
        //         $scope.gridOptions.selectItem(index, false);
        //         $scope.fieldAttributes.splice(index, 1);

        //         $scope.selectedSignals.length = 0;
        //         if (index < $scope.fieldAttributes.length) {
        //             $scope.gridOptions.selectItem(index + 1, true);
        //         } else if ($scope.fieldAttributes.length > 0) {
        //             $scope.gridOptions.selectItem(index - 1, true);
        //         }
        //     }
        // };
        ...
        $scope.save = function () {
            ...
            if ($scope.fieldAttributes.length > 0) {
                $scope.property.value = $scope.fieldAttributes;
            } else {
